...
def assign_role_permission():
    ...
    # Validate that role, module, and action exist and belong to the correct company
    ...
    module = Module.query.filter_by(id=data['module_id'], company_id=company_id).first()
    if not module:
        return jsonify({'error': 'Module not found in this company'}), 404
        
    action = ModuleAction.query.filter_by(id=data['action_id'], company_id=company_id).first()
    if not action:
        return jsonify({'error': 'Module action not found in this company'}), 404
...
def assign_user_permission():
    ...
    # Validate that user, role, module, and action exist and belong to the correct company
    user = User.query.filter_by(id=data['user_id'], company_id=company_id).first()
    if not user:
